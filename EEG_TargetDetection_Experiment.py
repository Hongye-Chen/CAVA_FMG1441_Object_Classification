...
blank = visual.Rect(
    ...
    width=(2736, 1824)[0], height=(2736, 1824)[1],
...
Images = visual.ImageStim(
    ...
    ori=0.0, pos=(0, 0), size=[2736,1824],
...
max_trial = 4       #The maximum of trials
...
Repetition = data.TrialHandler(nReps=1.0, method='sequential', 
...
for thisRepetition in Repetition:
    ...
    for thisBlock in Blocks:
        ...
        for thisTrial in Trials:
            ...
            #each element should contain a full filename not just number,
            #The code below loads the corresponding row of trial from the matrix,
            #please change the path below
            df = pd.read_csv(r'C:\Users\15202\OneDrive\C_\University of Amsterdam\Intern\Randomized matrix.csv', header = None, skiprows = list(range(0,eeg_trial_num)) + list(range(eeg_trial_num+1,max_trial)) )
            ...
            #please change the path below
            cur_correct = pd.read_csv(r'C:\Users\15202\OneDrive\C_\University of Amsterdam\Intern\Correction_answers.csv', header = None, skiprows = list(range(0,eeg_trial_num)) + list(range(eeg_trial_num+1,max_trial)) )
            cur_correct = cur_correct[0][0]     #Get the correct answer
            eeg_trial_num += 1      #Increase the trial index
            
            cur_trial_data = {'s1' : df.loc[0]}     
            df2 = pd.DataFrame(cur_trial_data)      #Convert the sequence into DataFrame
            
            #Save the current images list into the loop file
            df2.to_csv(r'C:\Users\15202\OneDrive\C_\University of Amsterdam\Intern\loopTemplate.csv',index=False)
            ...
            # set up handler to look after randomisation of conditions etc
            Sequences = data.TrialHandler(nReps=1.0, method='fullRandom', 
                extraInfo=expInfo, originPath=-1,
        ...
        num_correct = round( (num_correct / 4) * 100 , 2)       #Calculate accuracy rate
...
# completed 1.0 repeats of 'Repetition'
