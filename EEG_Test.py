...
expName = 'EEG_Test'  # from the Builder filename that created this script
expInfo = {
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\15202\\OneDrive\\C_\\University of Amsterdam\\Intern\\EEG_Test.py',
...
blank_screen_2 = visual.Rect(
    win=win, name='blank_screen_2',units='pix', 
...
for thisRepetition in Repetition:
    ...
    for thisBlock in Blocks:
        ...
        for thisTrial in Trials:
            ...
            # keep track of which components have finished
            Load_current_trial_infoComponents = [blank_screen_2, cross_2]
            ...
            while continueRoutine:
                ...
                # *blank_screen_2* updates
                if blank_screen_2.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                    # keep track of start time/frame for later
                    blank_screen_2.frameNStart = frameN  # exact frame index
                    blank_screen_2.tStart = t  # local t and not account for scr refresh
                    blank_screen_2.tStartRefresh = tThisFlipGlobal  # on global time
                    win.timeOnFlip(blank_screen_2, 'tStartRefresh')  # time at next scr refresh
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'blank_screen_2.started')
                    blank_screen_2.setAutoDraw(True)
                if blank_screen_2.status == STARTED:
                    if frameN >= (blank_screen_2.frameNStart + 1):
                        # keep track of stop time/frame for later
                        blank_screen_2.tStop = t  # not accounting for scr refresh
                        blank_screen_2.frameNStop = frameN  # exact frame index
                        # add timestamp to datafile
                        thisExp.timestampOnFlip(win, 'blank_screen_2.stopped')
                        blank_screen_2.setAutoDraw(False)
                ...
                if cross_2.status == STARTED:
                    if frameN >= (cross_2.frameNStart + 1):
                        # keep track of stop time/frame for later
            ...
            for thisSequence in Sequences:
                ...
                # update component parameters for each repeat
                # keep track of which components have finished
                Preload_and_Blank_ScreenComponents = [Images, blank_screen, cross]
            ...
            while continueRoutine:
                ...
                # *text* updates
                if text.status == NOT_STARTED and tThisFlip >= len_blank_short-frameTolerance:
                    # keep track of start time/frame for later
                ...
                # *key_resp* updates
                waitOnFlip = False
                if key_resp.status == NOT_STARTED and tThisFlip >= len_blank_short-frameTolerance:
                    # keep track of start time/frame for later
        ...
        end_text.setText(block_end_text)
        key_resp_2.keys = []
        key_resp_2.rt = []
        _key_resp_2_allKeys = []
        # keep track of which components have finished
        EndComponents = [end_text, text_2, key_resp_2]
